...
const values = data => data.filter((d) => d.covid_deaths_daily).map(d => d.covid_deaths_daily);
const order = data => data.filter((d) => d.covid_deaths_daily).map(d => d.d).map(ordinal);
const latest = data =>
